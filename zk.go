		...
		for {
			...
			default:
				...
				for len(data) >= 12 {
					...
					var attType int = unpack[2].(int)
					c <- &Attendance{UserID: userID, AttendedAt: timestamp, AttType: attType}
					log.Printf("UserID %v timeStamp %v AttendType %v \n", userID, timestamp, attType)
				}
			}
		}
